...
class LoginForm extends TPage
{
    ...
    function __construct($param)
    {
        ...
        $table1->style = "border-collapse:collapse;width:100%";

        $table2 = new TTable;
        $table2->border = 0;
        $table2->cellpadding = '2';
        $table2->style = "border-collapse:collapse;width:70%";
        ...
        $escola_id  = new TDBCombo('escola_id','platia','Colegio','id','nome');
        ...
        $row = $this->form->addFields( [$instancia, $escola_id] );
        $row->layout = ['col-sm-12 display-flex'];
        ...
        $logo->style = 'text-align:center; margin-bottom:20px;';
        $logo->add('<img src="app/images/Logo_PlanoAula_1.png" style="max-width:160px;">');

        $wrapper = new TElement('div');
        $wrapper->style = 'margin:auto; margin-top:100px;max-width:460px;';
        $wrapper->id    = 'login-wrapper';

        // adiciona logo acima do form
        $wrapper->add($logo);
        ...
        $h3 = new TElement('h1');
        $h3->style = 'margin-top:100px; line-height: 1.8; text-align:center; color:#007FFF';
        $h3->add('Login');
        $img = new TImage('app/images/Logo_Platia_PC.png');
        $img->style = 'width:75%; display: block; margin-left: auto; margin-right: -120px; margin-top: 90px; float: right; box-shadow: 0 4px 70px 0 rgba(0, 0, 0, 0.2), 0 6px 20px 0 rgba(0, 0, 0, 0.19);';

        $row = $table2->addRow();
        $row = $table2->addRow();
        $cell=$row->addCell($wrapper);
        ...
        $cell->{'style'}='width:20%; align: center';
        $row->addCell($table2);

        parent::add($table1);

        // inclui css customizado
        TPage::include_css('app/resources/login-custom.css');

        // adiciona wrapper na página
        //parent::add($wrapper);
    ...
    public static function onLogin($param)
    {
        ...
        try
        {
            ...
            /*if ($data->login!='admin')
              (new TRequiredValidator)->validate('Instancia Gestora', $data->instancia_id);
            else $data->instancia_id=1; //MEC*/
