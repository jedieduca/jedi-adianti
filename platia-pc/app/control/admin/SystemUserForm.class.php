...
class SystemUserForm extends TPage
{
    ...
    function __construct()
    {
        ...
        $groups        = new TDBCheckGroup('groups','platia','SystemGroup','id','name');
        ...
        $instancias = new TDBMultiSearch('instancias', 'platia', 'InstanciaGestora', 'id', 'nome');
    ...
    public function onSave($param)
    {
        try
        {
            ...
            $object->frontpage_id=41;  //força a página de inicialização do memore
            $object->store();
            $object->clearParts();
            $userId = $object->id;
    ...
    function onEdit($param)
    {
        try
        {
            if (isset($param['key']))
            {
                ...
                TTransaction::open('platia');
                $instancias  = array();
                if( $instancias_db = $object->getSystemUserInstancias() )
                {
                ...
                try
                {            
                   $objUt = new Utilizador($key);
                   $this->form->setData($objUt);
                }
                catch (Exception $e) // in case of exception
                {
                }
                ...
                try
                {
                    $objDocente = new Docente($key);
                    $disciplina_ids = array();
                    foreach ($object->getSystemDocenteDisciplina() as $disciplina)
                    {
                        $disciplina_ids[] = $disciplina->id;
                    }                   
                    $objDocente->disciplina_list = $disciplina_ids;
                    $this->form->setData($objDocente);
                }
                catch (Exception $e) // in case of exception
                {
                }
