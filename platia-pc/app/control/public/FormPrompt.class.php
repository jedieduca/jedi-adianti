...
class FormPrompt extends TPage
{
    ...
    function __construct()
    {
        ...
        $systemPrompt1 = new TText('system_prompt1');
        $systemPrompt1->placeholder = 'Digite aqui o System Prompt';
        //$systemPrompt->setTip('Tip for description');
        $systemPrompt1->setSize('100%', 160);

        $userPrompt1   = new TText('user_prompt1');
        $userPrompt1->placeholder = 'Digite aqui o User Prompt';
        $userPrompt1->setSize('100%', 200);

        $systemPrompt2 = new TText('system_prompt2');
        $systemPrompt2->placeholder = 'Digite aqui o System Prompt';
        //$systemPrompt->setTip('Tip for description');
        $systemPrompt2->setSize('100%', 160);
        ...
        $this->form->addFields( [new TLabel('System Prompt 1')],   [$systemPrompt1] );
        $this->form->addFields( [new TLabel('User Prompt (Passo 1)')],    [$userPrompt1] );
        $this->form->addFields( [new TLabel('System Prompt 2')],   [$systemPrompt2] );
    ...
    public function onSave($param)
    {
        ...
        if ((empty($data->id_eixo)) || (empty($data->system_prompt1)) || (empty($data->user_prompt1)))
        {
        ...
        { 
            ...
            $objPrompt->id_eixo = $data->id_eixo; 
            $objPrompt->system_prompt1 = $data->system_prompt1; 
            $objPrompt->user_prompt1 = $data->user_prompt1; 
            $objPrompt->system_prompt2 = $data->system_prompt2; 
