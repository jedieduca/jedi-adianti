...
class FormPerguntaList extends TStandardList
{
    ...
    public function __construct()
    {
        ...
        //$idTema     = new TDBCombo('idtema','platia','Tema','id','nome');
        $categoria  = new TDBMultiSearch('idCategorias', 'platia', 'Categoria', 'id', 'nome', 'nome');
    ...
    function onReload_old($param = NULL)   //sem filtro categoria
    {
        ...
        // run query
        $sql='DROP VIEW IF EXISTS pergunta2view ';
        $conn->query($sql);

        //$sql='CREATE VIEW tema2view AS select *';
        $sql='CREATE VIEW pergunta2view AS select p.id, p.idtema, p.pergunta ';
        ...
        if ((strlen(array_search(1,TSession::getValue('usergroupids')))==0) && 
            ...
            $sql.='AND t.idautor='.TSession::getValue('userid');
        ...
        if (TSession::getValue('formPerguntaList_filter_id')) {
            $criteria->add(TSession::getValue('formPerguntaList_filter_id')); // add the session filter
        }

        if (TSession::getValue('formPerguntaList_filter_noticia')) {
            $criteria->add(TSession::getValue('formPerguntaList_filter_noticia')); // add the session filter
        }

        if (TSession::getValue('formPerguntaList_filter_idCategorias')) {
            $criteria->add(TSession::getValue('formPerguntaList_filter_idCategorias')); // add the session filter
        }
    ...
    function onReload($param = NULL)
    {
        ...
        try
        {
            ...
            $sql .= 'SELECT p.id, p.idtema, p.pergunta ';
            $sql .= 'FROM pergunta2 p ';
            $sql .= 'LEFT JOIN tema2 t ON p.idtema = t.id ';
            $sql .= 'WHERE p.idtema = 17 '; // tema default Fake News
