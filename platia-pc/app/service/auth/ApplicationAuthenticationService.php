...
class ApplicationAuthenticationService
{
     ...
     */
    public static function setInstancia($instancia_id)
    {
        $ini  = AdiantiApplicationConfig::get();
        

            TSession::setValue('userunitid',   $instancia_id );
            TSession::setValue('userunitname', InstanciaGestora::findInTransaction('platia', $instancia_id)->nome);
    ...
    public static function setUnit($unit_id)
    {
        ...
        if (!empty($ini['general']['multiunit']) and $ini['general']['multiunit'] == '1' and !empty($unit_id))
        {
            ...
            TSession::setValue('userunitname', SystemUnit::findInTransaction('platia', $unit_id)->name);
            
            if (!empty($ini['general']['multi_database']) and $ini['general']['multi_database'] == '1')
            {
                TSession::setValue('unit_database', SystemUnit::findInTransaction('platia', $unit_id)->connection_name );
            }
        }
    }
