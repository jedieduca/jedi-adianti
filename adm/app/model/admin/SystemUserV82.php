 ...
 */
class SystemUserV82 extends TRecord
{
   ...
   /**
    ...
    {
        //echo '<pre>'; print_r('entrou'); echo '</pre>';
        // loads the associated object
        if (empty($this->unit))
            $this->unit = new Campus($this->system_unit_id);  //$this->unit = new SystemUnit($this->system_unit_id);
    ...
    /**
     * Add a Campus to the user
     * @param $object Instance of Campus
     */
    public function addSystemUserCampus(Campus $campus)
    {
        $object = new UsuarioCampus();
        $object->id_campus = $campus->id;
        $object->id_usuario = $this->id;
        $object->store();
    }
    ...
    public function getSystemUserRoleCodes( $as_string = false )
    {
        ...
        /**
     * Return the user' campus
     * @return Collection of Campus
     */
    public function getSystemUserCampus()
    {
        return parent::loadAggregate('Campus', 'UsuarioCampus', 'id_usuario', 'id_campus', $this->id);
    }

    /**
     * Get user campus ids
    */
    public function getSystemUserCampusIds( $as_string = false )
    {
        $unitids = array();
        $units = $this->getSystemUserCampus();
        if ($units)
        {
            foreach ($units as $unit)
            {
    ...
    public static function validate($login)
    {
        ...
        if ($user instanceof SystemUserV82)
        {
            if ($user->active == 'N')
            {
     ...
     * @returns SystemUserV82 if the password matches, otherwise throw Exception
     */
    ...
    {
        return SystemUserV82::where('login', '=', $login)->first();
    }
    ...
    {
        return SystemUserV82::where('id', '=', $id)->first();
    }
    ...
    {
        return SystemUserV82::where('custom_code', '=', $custom_code)->first();
    }
    ...
    {
        return SystemUserV82::where('email', '=', $email)->first();
    }
