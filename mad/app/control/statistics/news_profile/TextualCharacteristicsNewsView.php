...
class TextualCharacteristicsNewsView extends TStandardList
{
    ...
    */
    public function __construct()
    {
        ...
        parent::setActiveRecord('TextualCharacteristicsNews');             // defines the active record
        parent::setDefaultOrder('id', 'asc');                            // defines the default order
        parent::addFilterField('id', '=', 'id');                         // filterField, operator, formField
        parent::addFilterField('caracteristica_observada', '=', 'caracteristica_observada');           // filterField, operator, formField
        parent::setLimit(TSession::getValue(__CLASS__ . '_limit') ?? 10);

        parent::setAfterSearchCallback( [$this, 'onAfterSearch' ] );

        // creates the form
        $this->form = new BootstrapFormBuilder('form_search_TextualCharacteristicsNews');
        $this->form->setFormTitle(_t('Textual Characteristics of News'));

        // create the form fields
        $id        = new TEntry('id');
        $categoria = new TDBCombo('caracteristica_observada', 'jedi', 'TextualCharacteristicsNews', 'caracteristica_observada', 'caracteristica_observada');
        ...
        $this->form->addFields( [new TLabel(_t('Observed Characteristics'))], [$categoria] );
        ...
        $col_caract_obs = new TDataGridColumn('caracteristica_observada', _t('Observed Characteristics'), 'left');
        $col_fake       = new TDataGridColumn('fake', 'Fake', 'right');
        $col_nao_fake   = new TDataGridColumn('nao_fake', 'Não Fake', 'right');

        // add the columns to the DataGrid
        $this->datagrid->addColumn($col_id);
        $this->datagrid->addColumn($col_caract_obs);
        $this->datagrid->addColumn($col_fake );
        $this->datagrid->addColumn($col_nao_fake);
        ...
        $order_caract_obs = new TAction(array($this, 'onReload'));
        $order_caract_obs->setParameter('order', 'caracteristica_observada');
        $col_caract_obs->setAction($order_caract_obs);

        $order_fake = new TAction(array($this, 'onReload'));
        $order_fake->setParameter('order', 'fake');
        $col_fake->setAction($order_fake);

        $order_nao_fake = new TAction(array($this, 'onReload'));
        $order_nao_fake->setParameter('order', 'nao_fake');
        $col_nao_fake->setAction($order_nao_fake);

        // create EDIT action
        $action_view = new TDataGridAction(array('TextualCharacteristicsNewsForm', 'onView'), ['register_state' => 'false'] );
