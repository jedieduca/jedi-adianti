...
class StatisticsCategoryView extends TStandardList
{
    ...
    */
    public function __construct()
    {
        ...
        parent::setActiveRecord('StatisticsCategory');                // defines the active record
        ...
        // creates the form
        $this->form = new BootstrapFormBuilder('form_search_StatisticsCategory');
        ...
        $col_media_acertos = new TDataGridColumn('media_acertos', _t('Average number of correct answers'), 'right');
        $col_media_erros   = new TDataGridColumn('media_erros', _t('Error average'), 'right');

        // format the columns in the DataGrid
        $col_media_acertos->setTransformer( function($value, $object, $row) {
            return number_format($value, 2, ',');
        });

        $col_media_erros->setTransformer( function($value, $object, $row) {
            return number_format($value, 2, ',');
        });
        ...
        $this->datagrid->addColumn($col_media_acertos);
        $this->datagrid->addColumn($col_media_erros);
        ...
        $order_media_acertos = new TAction(array($this, 'onReload'));
        $order_media_acertos->setParameter('order', 'media_acertos');
        $col_media_acertos->setAction($order_media_acertos);

        $order_media_erros = new TAction(array($this, 'onReload'));
        $order_media_erros->setParameter('order', 'media_erros');
        $col_media_erros->setAction($order_media_erros);

        // create EDIT action
        $action_view = new TDataGridAction(array('StatisticsCategoryForm', 'onView'), ['register_state' => 'false'] );
    ...
    public function onReload($param = NULL)
    {
        ...
        try {
            ...
            if (isset($apiData['link_imagem']->grafico_categoria_turma)){
                ...
                $this->imageContainer = new TImage($apiData['link_imagem']->grafico_categoria_turma);
