...
class MatchSummaryView extends TStandardList
{
    ...
    */
    public function __construct()
    {
        ...
        parent::setActiveRecord('MatchSummary');             // defines the active record
        ...
        // creates the form
        $this->form = new BootstrapFormBuilder('form_search_MatchSummary');
        ...
        $col_media      = new TDataGridColumn('media', _t('Average'), 'right');
        $col_desvio     = new TDataGridColumn('desvio', _t('Standard Deviation'), 'right');

        // format the columns in the DataGrid
        $col_media->setTransformer( function($value, $object, $row) {
            return number_format($value, 2, ',');
        });

        $col_desvio->setTransformer( function($value, $object, $row) {
            return number_format($value, 2, ',');
        });
        ...
        $this->datagrid->addColumn($col_media);
        ...
        $order_caract_obs->setParameter('order', 'caracteristicas_observadas');
        $col_caract_obs->setAction($order_caract_obs);

        $order_media = new TAction(array($this, 'onReload'));
        $order_media->setParameter('order', 'media');
        $col_media->setAction($order_media);

        $order_desvio = new TAction(array($this, 'onReload'));
        $order_desvio->setParameter('order', 'desvio');
        $col_desvio->setAction($order_desvio);

        // create EDIT action
        $action_view = new TDataGridAction(array('MatchSummaryForm', 'onView'), ['register_state' => 'false'] );
